...
export const ProductsTable = () => {
    ...
    const data = products.map((product, index) => 
            ...
            <li className={`flex p-2 ${(index+1) % 2 ? "bg-white" : "bg-slate-200"}`} key={index}>
                ...
                <div className="w-1/5 text-center flex justify-center content-center"> <img className="size-10" src={product.productImage} /> </div>
                <div className={`w-1/5 text-center font-plus-font text-sm font-medium text-ellipsis overflow-hidden capitalize`}> {product.productName} </div>
                <div className="w-1/5 text-center font-plus-font"> {product.productPrice} </div>
